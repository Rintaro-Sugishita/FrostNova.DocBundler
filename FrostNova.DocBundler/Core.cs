...
public static partial class Core
{
    ...
    public static string Resolve(string filePath, string root, HashSet<string> visited, bool embedImages)
    {
        ...
        try
        {
            ...
            foreach (var line in lines)
            {
                ...
                var processedLine = ImageRegex().Replace(line, m =>
                {
                    ...
                    // 絶対パスの解決
                    var absImagePath = path.StartsWith("/")
                        ? Path.GetFullPath(path.TrimStart('/'), root)
                        : Path.GetFullPath(Path.Combine(currentDir, path));

                    // --- ここから追加・修正 ---
                    if (embedImages) // ← 事前に static フィールド等で保持しているフラグ
                    {
                        return EmbedImageAsBase64(absImagePath, alt, m.Value);
                    }
                    else
                    {
                        // 既存の相対パス書き換えロジック
                ...
                if (match.Success)
                {
                    ...
                    // インポート先の内容を取得
                    var importedContent = ProcessContent(fullImportPath, ext, root, visited, embedImages);
    ...
    private static string ProcessContent(string path, string ext, string root, HashSet<string> visited, bool embedImages)
    {
        return ext switch
        {
            ".md" => Resolve(path, root, visited, embedImages),
            ".csv" => ConvertCsvToMdTable(File.ReadAllLines(path)),
            ".cs" or ".ts" or ".sql" or ".json" => $"```{ext.TrimStart('.')}\n{File.ReadAllText(path)}\n```",
            _ => Resolve(path, root, visited, embedImages)
        };
    }
    ...
    internal static string EmbedImageAsBase64(string absPath, string alt, string defaultValue)
    {
        try
        {
            if (!File.Exists(absPath)) return defaultValue;

            var ext = Path.GetExtension(absPath).ToLower().TrimStart('.');
        ...
        {
            return defaultValue; // 失敗時は元の文字列を返す
        }
    }
